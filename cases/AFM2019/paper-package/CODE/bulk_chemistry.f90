...
!!
  ...
  double precision :: delta_coo18, wdeltacoo18_p, wdeltacoo18_r, wdeltacoo18, wc18_p, wc18_r
...
!   a0= 0.653289 a0 = 0.8775 (sim04) a0 = 0.42458 (sim05)
  ...
  close(1)
  open (1, file='namoptions')
  read (1,NAMISOTOPES,iostat=ierr)
  if (ierr > 0) stop 'ERROR: Problem in namoptions'
  close(1)
  open (1, file='namoptions')
  ...
  if ( lisotopes .and. (.not. lrsAgs )) then
    print *,""
    print *,""
    print *,"!!!!!!!!!!!!!!!!!!!!!"
    print *,""
...
!    plant 
        ...
        eps_k13        =  4.4*rsCO2/(ra + rsCO2)                                        ! Eq. (A.20) rb=0                                                           ! permil
        deltac13       =  eps_k13 + (bc13 - eps_k13)*(cippb/CO2ISO)                     ! Eq. (A.19)                          ! permil 
        wdeltac13_p    = -(((An*1000.)*(MW_Air/MW_CO2)*(1.0/rho))  /CO2ISO)*deltac13   ! Eq. (A.18) Flux permil m s-1
...
!   wc13 in ppb m/s by plant (wc13_p), soil (wc13_r) and total Q_cbl(C13%loc)
        ...
        Q_C13          = (C13_BL/CO2ISO)*(wco2*1000) + (Rsc13/1000.) * CO2ISO * wdeltac13    ! Eq. (3) ppb m s-1
       endif !lC13
...
!    plant 
        ...
        LEvegppb       =  (LEveg/(rho*Lv))*(MW_Air/MW_H2O)*1e9                            ! Eq. (A.36)                                 ! ppb m/s
        wdeltahoo18_p  =  (LEvegppb/H2OISO)  * (delta_xw - delta_atmhoo18)                ! permil m/s

!    soil
        LEsoilppb      =  (LEsoil/(rho*Lv))*(MW_Air/MW_H2O)*1e9                           ! ppb m/s
        wdeltahoo18_r  =  (LEsoilppb/H2OISO) * (delta_xw - delta_atmhoo18)                ! Eq. (A.37) permil m/s
...
!      plant 
          ...
          eps_eq           = (1 - (1./alpha_eq)) * 1000.                                   ! Eq. (A.28) permil 
          eps_kw           = 32.*rsAgs/(ra + rsAgs)                                        ! Eq. (A.27) rb=0 permil NB:we assume r_b equal 0
          eps_k18          = 8.8*rsCO2/(ra + rsCO2)                                        ! Eq. (A.30) rb=0 permil NB:we assume r_b equal 0
...
!         JV Changing delta_atmcoo18 to delta_atmhoo18 water leaf calculation 10-05-2018 
          ...
          delta_lw         = delta_x + eps_eq + eps_kw + rhsurf*(delta_atmhoo18 - eps_kw - delta_x) * alpha_eq ! Eq. (A.26) permil

          delta_e18        = delta_lw + (17604/Ts) - 17.93                                        ! Eq. (A.25) permil
          ...
          delta_coo18      = (ccsppb/(ccsppb - CO2ISO))*(delta_e18 - delta_atmcoo18)*theta_eq - &
                             (1 - theta_eq) * eps_k18 * (ccsppb/CO2ISO) + eps_k18                 ! Eq. (A.24) permil 
...
!    emission and entrainment each timestep
    if(lisotopes) then
      if (lC13) then
        E_C13  = -(we+wf)*(C13_FT-C13_BL)   !Eq. (1)
    ...
    if(lchem) then
      do k=1, nchsp
        ...
        if (k == 32) then
             c_ft( k)= 17644.- 5.7e00*(zi(1)-zi0)  ! value 5.7 is the gamma_ho180 
       else
